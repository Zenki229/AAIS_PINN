...
class SAIS:
    def __init__(self, net, N1: int, N2: int, p0, max_update, pde, res_tol):
        self.N1 = N1
        self.N2 = N2
        self.p0 = p0
    ...
    def limit_state(self, node):
        pred = self.pde.solve(node=node, mode="rhs")
        return torch.abs(self.pde.residual(node=node, net=self.net, pred=pred, mode="ele", cls="in"))-self.res_tol

    def forward(self, dev):
        num_p = int(np.floor(self.p0*self.N1))
        node_sample = self.pde.sample(size=[self.N1, 0])["in"].detach().cpu().numpy()
        while self.iter <= self.max_update:
            res_node = self.limit_state({"in": torch.from_numpy(node_sample).to(device=dev)}).detach().cpu().numpy()
            ind = np.argsort(res_node, axis=None)[::-1]
            node_sort = node_sample[ind, :]
            res_sort = res_node[ind]
            node_fail = node_sort[res_sort>0, :]
            num_fail = node_fail.shape[0]
            if num_fail < num_p:
                hk_mean = np.mean(node_sort[:num_p, :], axis=0)
                hk_cov = np.cov(node_sort[:num_p, :].T)
                hk = ss.multivariate_normal(mean=hk_mean, cov=hk_cov)
                self.iter += 1
            else:
                break
            if "Box" in self.domain.__class__.__name__:
                if self.domain.dimension == 2:
                    node_sample = self.trunc_2Dnormal_box(gaussian=hk, size=self.N1, domain=self.domain)
                elif self.domain.dimension == 1:
                    node_sample = self.trunc_1Dnormal_box(gaussian=hk, size=self.N1, domain=self.domain)
                else:
                    raise ValueError("Invalid dimension")
            else:
                raise ValueError("Invalid shape of domain")
        mean_opt = np.mean(node_sort[:num_p, :], axis=0)
        cov_opt = np.cov(node_sort[:num_p, :].T)
        h_opt = ss.multivariate_normal(mean=mean_opt, cov=cov_opt)
        if "Box" in self.domain.__class__.__name__:
            if self.domain.dimension == 2:
                xs = self.domain.xlim[0]
                xe = self.domain.xlim[1]
                ys = self.domain.ylim[0]
                ye = self.domain.ylim[1]
                prob_domain = h_opt.cdf((xe,ye))-h_opt.cdf((xe,ys))-h_opt.cdf((xs,ye))+h_opt.cdf((xs,ys))
                node_opt = self.trunc_2Dnormal_box(gaussian=h_opt, size=self.N2, domain=self.domain)
                weight = np.ones(shape=(self.N2,)) / ((xe-xs) * (ye-ys))
            elif self.domain.dimension == 1:
                xs = self.domain.xlim[0]
                xe = self.domain.xlim[1]
                prob_domain = h_opt.cdf(x=xe)-h_opt.cdf(x=xs)
                node_opt = self.trunc_1Dnormal_box(gaussian=h_opt,size=self.N2, domain=self.domain)
                weight = np.ones(shape=(self.N2,))/(xe-xs)
            else:
                raise ValueError("Invalid dimension")
        else:
            raise ValueError("Invalid shape of domain")
        weight2 = h_opt.pdf(x=node_opt)/prob_domain
        fail_region = np.ones(shape=(self.N2,))
        res_node = self.limit_state({"in": torch.from_numpy(node_opt).to(device=dev)}).detach().cpu().numpy()
        fail_region[res_node <= 0] = 0
        prob_fail = np.mean(weight*fail_region/weight2)
        node_fail = node_opt[res_node > 0, :]
        return prob_fail, torch.from_numpy(node_fail).to(device=dev)

    def trunc_2Dnormal_box(self, gaussian, size, domain):
        node = gaussian.rvs(size=size)
        xs = domain.xlim[0]
        xe = domain.xlim[1]
        ys = domain.ylim[0]
        ye = domain.ylim[1]
        node_inside = node[(node[:, 0] > xs) & (node[:, 0] < xe)
                           & (node[:, 1] > ys) & (node[:, 1] < ye), :]
        N_res = size - node_inside.shape[0]
        while N_res > 0:
            if N_res > 5:
                node = gaussian.rvs(size=N_res)
                try:
                    node_inside = np.concatenate(
                        [node_inside,
                         node[(node[:, 0] > xs) & (node[:, 0] < xe)
                              & (node[:, 1] > ys) & (node[:, 1] < ye), :]],
                        axis=0)
                except:
                    continue
            else:
                node = gaussian.rvs(size=5)
                try:
                    node_inside = np.concatenate(
                        [node_inside,
                         node[(node[:, 0] > xs) & (node[:, 0] < xe)
                              & (node[:, 1] > ys) & (node[:, 1] < ye), :]],
                        axis=0)
                    node_inside = node_inside[:size, :]
                except:
                    continue
            node_inside = np.unique(node_inside, axis=0)
            N_res = size - node_inside.shape[0]
        return node_inside

    def trunc_1Dnormal_box(self, gaussian, size, domain):
        node = gaussian.rvs(size=size)
        xs = domain.xlim[0]
        xe = domain.xlim[1]
        node_inside = node[(node > xs) & (node < xe)]
        N_res = size - node_inside.shape[0]
        while N_res > 0:
            if N_res > 5:
                node = gaussian.rvs(size=N_res)
            ...
            node_inside = np.unique(node_inside, axis=0)
            N_res = size - node_inside.shape[0]
        return node_inside.reshape(-1, 1)

...
class Heat2D:
    ...
    def solve(self, mode, node):
        ...
        if mode == "in":
            ...
            node_init = node
            val_init = torch.sin(self.case*np.pi*node[:, 1])*torch.sin(self.case*np.pi*node[:, 2])
            return val_init
        else:
            raise ValueError("invalid mode")

    def exact(self, node):
        return np.exp(-node[:, 0])*np.sin(self.case*np.pi*node[:, 1])*np.sin(self.case*np.pi*node[:, 2])
    ...
    def residual(self, node, net, cls="loss", mode="in"):
        ...
        if mode == "in":
            ...
            if cls == "loss":
                pde_res = self.criterion(dt.flatten(), (dxx+dyy)/(2*np.power(self.case*np.pi,2)))
            elif cls == "ele":
                pde_res = dt.flatten()-(dxx+dyy)/(2*np.power(self.case*np.pi, 2))
            else:
                raise ValueError("Invalid mode")
            return pde_res
        elif mode == "bd":
    ...
    def test_err_plot(self, net, path, num):
        ...
        for enum, t_now in enumerate(t_plt):
            ...
            plot = axes[enum, 1].pcolormesh(mesh_x, mesh_y, exact_t.reshape(mesh_x.shape), shading='gouraud',
                                            cmap='jet')
            axes[enum, 1].set_title(f'exact')
            fig.colorbar(plot, ax=axes[enum, 1], format="%1.1e")
            plot = axes[enum, 2].pcolormesh(mesh_x, mesh_y, err_t_plt.reshape(mesh_x.shape), shading='gouraud',
                                            cmap='jet')
